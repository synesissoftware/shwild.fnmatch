...
if [ $RunMake -ne 0 ]; then
  ...
  if [ $ListOnly -eq 0 ]; then
    ...
    echo "Executing build (via command \`${MakeCmdClr}\`) and then running all ${ProjectNameClr} scratch (and performance) test programs"
...
if [ $status -eq 0 ]; then
  ...
  if [ $ListOnly -ne 0 ]; then
    ...
    echo "Listing all ${ProjectNameClr} scratch (and performance) test programs"
  else

    echo "Running all ${ProjectNameClr} scratch (and performance) test programs"
  fi
